...
func (wh *WebHandlers) UsersHandler(w http.ResponseWriter, r *http.Request) {
	rows, err := wh.db.Query("SELECT user_id, email, mobile_number, user_type_id, is_active, created_at, last_login, last_accessed_from FROM users")
	if err != nil {
...
func (wh *WebHandlers) UserEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		var u User
		err := wh.db.QueryRow("SELECT user_id, email, mobile_number, user_type_id, is_active, created_at, last_login, last_accessed_from FROM users WHERE user_id = ?", id).
			Scan(&u.ID, &u.Email, &u.MobileNumber, &u.UserTypeID, &u.IsActive, &u.CreatedAt, &u.LastLogin, &u.LastAccessedFrom)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		tmpl, tmplErr := wh.ExecuteTemplate("user_form", u)
		if tmplErr != nil {
